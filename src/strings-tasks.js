...
function removeFirstOccurrences(str, value) {
  ...
  if (position !== -1) {
    const x = str.substring(0, position);
    const y = str.substring(position + value.length);
    return x.concat(y);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  ...
  return x !== -1
    ? str.substr(0, x) + str.substr(x + value.length, str.length)
    : str;
}
...
function isPalindrome(str) {
  const ans = str.replaceAll(/\W/g, '').toLowerCase();
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    let maxChar = 0;
    if (code >= 'A'.charCodeAt(0) && code <= 'Z'.charCodeAt(0)) {
      maxChar = 'Z'.charCodeAt(0);
    } else if (code >= 'a'.charCodeAt(0) && code <= 'z'.charCodeAt(0)) {
      maxChar = 'z'.charCodeAt(0);
    }
    const newCode = code + 13 > maxChar ? code - 13 : code + 13;
    ans += maxChar ? String.fromCharCode(newCode) : str[i];
  }
  return ans;
}
...
function getCardId(value) {
  const x1 = ['♣', '♦', '♥', '♠'];
  const x2 = ['A', '2', '3', '4', '5', '6', '7', '8', '9', '1', 'J', 'Q', 'K'];
  return x1.indexOf(value[value.length - 1]) * 13 + x2.indexOf(value[0]);
}
